...
class b2c_ctl_site_product extends b2c_frontpage{
    ...
    private function _get_goods_setting($goods_id){
        ...
        $setting['goodsbn'] = $this->app->getConf('goodsbn.display.switch');//是否启用货品编号
        $setting['productsbn'] = $this->app->getConf('productsbn.display.switch');//是否启用商品编号
    ...
    public function index() {
        ...
        //echo '<pre>';print_r($productBasic);exit();
    ...
    function goodsSellLoglist($gid,$nPage=0){
        ...
        $selllog_member_price = $this->app->getConf('selllog.display.member.price');
        $this->pagedata['selllog_member_price']= $selllog_member_price;
        $nPage = $nPage?$nPage:1;
    ...
    function ajax_product_price($product_id){
        ...
        if(!cachemgr::get('ajax_product_price'.$product_id,$price)){
            ...
            if($prepare)
            {
                $pre=$prepare->getSpecialProduct($product[0]['product_id']);

                if(!empty($pre)&&$pre['promotion_type']=='prepare')
                {
                    $pre['nowtime']=time();
                    $productBasic['prepare'] = $pre;
                    $productBasic['promotion_type'] = $pre['promotion_type'];
                    $price['prepare_price']=$pre['preparesell_price'];
                }
            }
    ...
    function _get_product_basic($productId,$aGoods,$siteMember){
        ...
        $productBasic['promotion'] = $productPromotion;//商品促销
         ...
         //获取预售信息
        //$prepareProModl= app::get('preparesell')->model('preparesell_goods');
        //$prepare=$prepareProModl->getRow('*',array('product_id'=>$productId));
        $prepare=kernel::service('prepare_prepare');
    ...
    function toNotify(){
        ...
        if(!empty($_POST['cellphone']) && !preg_match('/^1[3|5|7|8][0-9]\d{8}$/', $_POST['cellphone'])){
