...
class b2c_ctl_site_cart extends b2c_frontpage{
    ...
    public function checkout_result($isfastbuy=0){
        ...
        /* 是否开启配送时间的限制 */
        $receivermore = $this->app->getConf('site.checkout.receivermore.open');
        $this->pagedata['site_checkout_special'] = $this->app->getConf('site.checkout.special');
        $this->pagedata['site_checkout_receivermore_open'] = $receivermore;
        $hasNight = $this->app->getConf('site.checkout.hasnight') == 'true' ? true : false;
        $this->pagedata['datetable_options'] = json_encode(array('type' => 'datetable','hasNight' => $hasNight,'shortest' => $this->app->getConf('site.checkout.shortest')));
        if($def_addr['special'] == 'true' && $receivermore == 'true'){
        ...
        if($this->pagedata['shipping_method']){
            ...
            //预售信息的判断，排除线下支付
            if($arrproducts && $arrproducts['promotion_type']=='prepare')
            {
        ...
        // 是否可以用积分抵扣
        if($arrproducts['promotion_type']!='prepare' || !$arrproducts)
        {
       ...
       /* $objproducts = $this->app->model('products');
        ...
        if($prepare)
        {
         ...
         //echo '<pre>';print_r($this->pagedata['payments']);exit();
    ...
    public function _common($flag=0,$is_fastbuy=false) {
        ...
        $aCart['apply_platform'] = $aData['apply_platform'];
