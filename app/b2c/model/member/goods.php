...
###删除收藏商品
	 ...
	 function count($filter=null,$type=null){
		...
		$count = $this->db->selectrow("SELECT COUNT(member_goods.`goods_id`) AS num
									FROM ".$this->table_name(1)." AS member_goods
									INNER JOIN ".$oGood->table_name(1)." AS goods ON member_goods.`goods_id`=goods.`goods_id`
									WHERE member_goods.`member_id`=".intval($filter['member_id'])." AND member_goods.`type`='". $type ."' AND goods.`marketable` IN ('true', 'false')");
        ...
        if(is_array($agid)&&$agid){
            ...
            if($aProduct){
                foreach ($aProduct as &$val) {
                    ...
                    $temp = $objProduct->getList('product_id, spec_info, price, freez, store, goods_id',array('goods_id'=>$val['goods_id']),$offset=0, $limit=-1,$orderType='price DESC');
                    if( $member_lv_id ){
                        //货品会员价
                        $tmpGoods = array();
