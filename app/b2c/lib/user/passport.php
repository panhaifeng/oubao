...
class b2c_user_passport
{
    ...
    //设置当前用户名
    public function set_local_uname($fmt,&$msg){
      $local_uname = strtolower($fmt['local_name']);
      ...
      if($membersData['local'] && $membersData['loginstyle']=='common'){
      ...
      if($fmt['logindisplay'])
      {
        $logindisplay = $fmt['logindisplay'];
        $data['logindisplay'] = $logindisplay;
      }
      $data['createtime'] = $row['createtime'];

      if( $pamMembersModel->save($data)  ){
    ...
    public function change_member_name($member_id, $member_name, &$msg){
        $member_name_local = $this->obj_filter->check_input($member_name);
        if($member_name_local != $member_name)
        {
            $msg = app::get('b2c')->_('请输入合法的用户名');
            return false;
        }
        if(!$this->check_signup_account($member_name,$msg))
        {
            return false;
        }
        $obj_pam_members = app::get('pam')->model('members');
        $memberData      = $obj_pam_members->getRow('*', array('member_id'=>$member_id,'login_type'=>'local'));
        if(!$obj_pam_members->is_openIdNameByName($memberData['login_account']))
        {
            $msg = app::get('b2c')->_('无法修改原有用户名');
            return false;
        }
        $memberData['login_account'] = $member_name;
        $obj_pam_members->save($memberData, true);
        return true;
    }
