...
class b2c_finder_orders{
    ...
    public function detail_basic($order_id)
    {
        ...
        else
        {
            ...
            $aOrder['payinfo']['pay_app_id'] = $arr_payments['app_display_name'] ? $arr_payments['app_display_name'] : $payid;
        }
        ...
        if( $aOrder['addon'] != null )
        {
            $order_addon = unserialize($aOrder['addon']);
            $aOrder['order_chgpointmoney'] = $order_addon['order_chgpointmoney'] ? $order_addon['order_chgpointmoney'] : 0;
        }
    ...
    public function detail_mark($order_id)
    {
        ...
        $aOrder = $order->dump($order_id,'status,mark_text,mark_type');
        if ($aOrder['mark_text'])
        {
            $aOrder['mark_text'] = unserialize($aOrder['mark_text']);
        }
        $render->pagedata['status'] = $aOrder['status'];
