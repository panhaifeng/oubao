...
class b2c_order_full{
    ...
    public function get($order_id)
    {
        ...
        $arr_data['pay_status'] = (!$sdf['pay_status']) ? 'PAY_NO' : $arr_pay_status[$sdf['pay_status']];
        $arr_data['ship_status'] = (!$sdf['ship_status']) ? 'SHIP_NO' : $arr_ship_status[$sdf['ship_status']];
        ...
        if($sdf['ship_status'] != 0)
        {
            $obj_delivery = app::get('b2c')->model('delivery');
            $deliverys = $obj_delivery->getList('logi_name,logi_no,t_begin,memo', array('order_id'=>$order_id));
            foreach($deliverys as $delivery)
            {
                $arr_data['logistics_no'] .= $delivery['logi_no'].',';
                $arr_data['company_name'] .= $delivery['logi_name'].',';
                $arr_data['consign_time'] .= date('Y-m-d H:i:s', $delivery['t_begin']).',';
            }
            $arr_data['logistics_no'] = substr($arr_data['logistics_no'],0,-1);
            $arr_data['company_name'] = substr($arr_data['company_name'],0,-1);
            $arr_data['consign_time'] = substr($arr_data['consign_time'],0,-1);
        }
        if ($sdf['order_pmt'])
        {
        ...
        if( $bill_list )
        {
            ...
            {
                $payment_info = $obj_payments->getList('*', array('payment_id' => $row['bill_id'],'status'=>array('succ','progress')));
                if( $payment_info[0] )
                {
        ...
        if(($length = count($mark_text)) > 0){
            $memo = $mark_text[$length-1]['mark_text'];
            $arr_data['trade_memo'] = $memo;
        }
