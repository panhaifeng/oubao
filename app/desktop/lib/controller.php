...
class desktop_controller extends base_controller{
    ...
    public function certcheck(){
        if( $this->certcheck && !app::get('desktop')->getConf('activation_code_check') ){
            $params['type'] = pam_account::get_account_type('desktop');
            //检查证书是否合法,从而判定产品功能是否可用。比如b2c功能
            $certCheckObj = kernel::service("product_soft_certcheck");
            if(is_object($certCheckObj) && method_exists($certCheckObj,"listener_login"))
                $result = $certCheckObj->listener_login($params);
        }
    }
