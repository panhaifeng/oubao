 ...
 */
final class wap_payment_plugin_wapupacp extends ectools_payment_app implements ectools_interface_payment_app {

	/**
	 * @var string 支付方式名称
	 */
    public $name = '中国银联端网关支付（wap）';
    /**
     * @var string 支付方式接口名称
     */
	 ...
	 * @var string 统一显示的名称
	 */
    public $display_name = '中国银联网关支付（wap）';
    /**
	 * @var string 货币名称
	 */
    public $curname = 'CNY';
    ...
    public function __construct($app){
		...
		$this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_wapupacp', 'callback');
		...
		$this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_wapupacp', 'callback');
    ...
    public function setting(){
        return array(
                    ...
                    'sdk_sign_cert_path'=>array(
                        'title'=>app::get('wap')->_('签名证书'),
                        'type'=>'file',
                        'validate_type' => 'required',
                        'label'=>app::get('wap')->_('文件后缀名为.pfx'),
                    ),
                    'sign_cert_pwd'=>array(
                        'title'=>app::get('wap')->_('签名证书密码'),
                        'type'=>'string',
						'validate_type' => 'required',
                    ),
    ...
    public function dopay($payment){
        // 初始化日志
        $this->sdk_sign_cert_pwd = trim($this->getConf('sign_cert_pwd', __CLASS__));
